...
```cron
15 2 * * * cd /absolute/path/to/Skill\ RSI && /usr/bin/env node scripts/skill-rsi-cron-runner.mjs ux-design --agentic --real-eval --max-runs 20 --max-new-runs 1 --agent-model gpt-5.6-sol >> .skill-rsi/cron.log 2>&1
```
...
```text
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Set-Location 'C:\path\to\Skill RSI'; node scripts/skill-rsi-cron-runner.mjs ux-design --agentic --real-eval --max-runs 20 --max-new-runs 1 --agent-model gpt-5.6-sol"
```
