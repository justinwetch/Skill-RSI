...
test('ui api stores the selected project model across all model roles', async () => {
  ...
  for (const model of ['gpt-5.6-terra', 'gpt-5.6-luna', 'gpt-5.5']) {
    const project = await createProjectForUi({
      ...
      goal: 'Keep supported model selections.',
      model,
    });
    assert.equal(project.config.models.agent, model);
    assert.equal(project.config.models.generation, model);
    assert.equal(project.config.models.judge, model);
  }
});
