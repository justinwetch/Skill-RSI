...
```bash
node src/cli.js init my-skill --goal "Help agents write better..." --output text --model gpt-5.6-sol --target-iterations 3
...
node scripts/skill-rsi-cron-runner.mjs my-skill --agentic --real-eval --max-runs 20 --max-new-runs 1 --agent-model gpt-5.6-sol
```
