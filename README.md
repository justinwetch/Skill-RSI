...
- OpenAI model selection: `gpt-6-sol` by default, `gpt-6-astra` for the most demanding work, or `gpt-6-luna` for lower-cost iteration. Existing projects can still select the GPT-5.6, GPT-5.5, and GPT-5.4 Mini choices.
...
The current curated OpenAI choices use the [GPT-6 API model IDs](https://developers.openai.com/api/docs/models). The CLI also accepts a model ID directly for advanced use. Model availability can vary by API account; no model-backed run starts merely by changing the selection.
...
```bash
node scripts/skill-rsi-cron-runner.mjs ux-design --agentic --real-eval --max-runs 20 --max-new-runs 1 --agent-model gpt-6-sol
```
...
node src/cli.js init <name> --goal "..." --output text|code|code_visual --model gpt-6-sol --target-iterations 3
...
node src/cli.js run <project> --agentic --real-eval --loops 1 --model gpt-6-sol
...
node src/cli.js evaluate <project> \
  ...
  --output text \
  --gen-model gpt-6-sol --judge-model gpt-6-sol \
  --out result.json
